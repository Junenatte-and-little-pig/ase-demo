...
class DataLoader:
    ...
    def __call__(self, mag_coeff=100, sigma=1):
        ...
        full_mat = self.__gaussian_smoothing__(threeD_xyz, xyz_range, sigma)
        return full_mat
    ...
    def __get_3D_matrix__(self, full_list, xyz_range, mag_coeff):
        ...
        if hx - lx != 0:
            ...
            magnifier = int(mag_coeff) / (hy - ly)
        elif hz - lz != 0:
            magnifier = int(mag_coeff) / (hz - lz)
        else:
            magnifier = int(mag_coeff)
        dx, dy, dz = list(map(int,
                              [(hx - lx) * magnifier+1, (hy - ly) * magnifier+1,
                               (hz - lz) * magnifier+1]))
    ...
    def __gaussian_smoothing__(self, full_mat, xyz_range, sigma=1):
